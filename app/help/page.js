...
export default function HelpPage() {
  ...
  useEffect(() => {
    ...
    fetch("/api/communes", { cache: "no-store" })
      .then((response) => { if (!response.ok) throw new Error("تعذر تحميل البلديات"); return response.json(); })
      .then((data) => { if (active) setAllCommunes(Array.isArray(data.communes) ? data.communes : []); })
      .catch((error) => { console.error(error); if (active) setCommunesError(true); })
  ...
  async function handleSubmit(e) {
    ...
    const selectedCommune = communes.find((item) => item.nom_ar === form.municipality);
    const payload = { ...form, wilaya:selectedWilaya?.name_ar || form.wilaya, municipality:selectedCommune?.nom_ar || form.municipality, status:"new" };
    const { data, error } = await supabase.from("help_requests").insert([payload]).select("tracking_code").single();
    setLoading(false);
    if (error) { setResult({ success:false, message:`خطأ: ${error.message}` }); return; }
  ...
  return <main dir="rtl" style={mainStyle}><div style={boxStyle}><h1 style={{textAlign:"center"}}>🆘 طلب مساعدة</h1>
    {result?.success ? <div style={successStyle}><h2>✅ تم إرسال الطلب بنجاح</h2><p>{result.message}</p><p style={{color:"#666"}}>احتفظ برقم المتابعة لاستخدامه لاحقًا.</p></div> : <form onSubmit={handleSubmit}>
      ...
      <label>الولاية</label><select required name="wilaya" value={form.wilaya} onChange={handleChange} style={inputStyle}><option value="">اختر الولاية — 69 ولاية</option>{WILAYAS.map(w => <option key={w.code} value={w.code}>{w.code.padStart(2,"0")} — {w.name_ar}</option>)}</select>
      <label>البلدية</label><select required name="municipality" value={form.municipality} onChange={handleChange} style={inputStyle} disabled={!form.wilaya || loadingCommunes || communesError}>
        <option value="">{loadingCommunes ? "جاري تحميل البلديات..." : communesError ? "تعذر تحميل البلديات — أعد تحميل الصفحة" : form.wilaya ? `اختر البلدية — ${communes.length} بلدية` : "اختر الولاية أولًا"}</option>
        {communes.map(c => <option key={`${c.wilaya_num}-${c.code_postal}-${c.nom_ar}`} value={c.nom_ar}>{c.nom_ar}</option>)}
      </select>
      <label>نوع المساعدة المطلوبة</label><select required name="help_type" value={form.help_type} onChange={handleChange} style={inputStyle}><option value="">اختر نوع المساعدة</option><option value="مساعدة مالية">مساعدة مالية</option><option value="مساعدة غذائية">مساعدة غذائية</option><option value="مساعدة طبية">مساعدة طبية</option><option value="مساعدة في السكن">مساعدة في السكن</option><option value="مساعدة أخرى">مساعدة أخرى</option></select>
      ...
      <button type="submit" disabled={loading || loadingCommunes || communesError} style={{...buttonStyle,opacity:loading?0.6:1}}>{loading?"جاري الإرسال...":"إرسال طلب المساعدة"}</button>
    </form>}{result && !result.success && <p style={errorStyle}>{result.message}</p>}</div></main>;
}
...
const successStyle={textAlign:"center",padding:"30px 10px"}; const errorStyle={color:"red",textAlign:"center",marginTop:"20px"};
